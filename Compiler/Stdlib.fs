...
let initFuncTy = fn [ "init", gen "init" ] (tup [ gen "state"; cmd])
let receiveFuncTy = fn ["state", gen "state"; "msg", gen "msg"] (tup [ gen "state"; cmd])
let actorDef = strct ([ "init", initFuncTy; "receive", receiveFuncTy ], false, None)
let stdlibActors =
  stdlibModule
    [ ("spawn",
       fn
